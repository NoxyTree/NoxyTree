# NoxyTree

Small, local tools for dependable automation and creator workflows.

## Cadence Starter + Approval Guard

**Your script changed. Your pipeline should notice.**

Two tested local Python tools for automation that must stop when reviewed inputs change. Approval Guard adds content-bound gates to an existing pipeline; Cadence Starter also tracks dependencies, catches stale work, blocks unsafe progress, and names the next safe action. Neither tool requires Claude, an AI account, or a subscription.

![Cadence Starter approval-staleness walkthrough](https://raw.githubusercontent.com/NoxyTree/cadence-starter/main/assets/cadence-demo.gif)

- [See the real product behaviour](https://noxytree.github.io/cadence-starter/)
- [Inspect the exact paid delivery receipt](https://noxytree.github.io/cadence-starter/bundle-receipt/)
- [Read the free media-pipeline field notes](https://noxytree.github.io/cadence-starter/guides/)
- [Inspect the public preview repository](https://github.com/NoxyTree/cadence-starter)
- [Try the free approval-fingerprint sample](https://noxytree.github.io/cadence-starter/downloads/cadence-approval-sample.zip?utm_source=github_profile&utm_medium=profile&utm_campaign=free_sample)
- [Get Approval Guard for $1+ or both tools for $7+](https://ko-fi.com/s/a8645f925f)

Approval Guard is $1+ and the two-tool bundle is $7+. The complete delivery contains 72 files and 31 passing tests across both tools, with readable source, documentation, examples, checksums and a single-operator commercial-use licence.

## Free field notes

- [Approval gates for AI-assisted media pipelines](https://noxytree.github.io/cadence-starter/guides/approval-gates-ai-media-pipelines/)
- [A practical faceless-video workflow](https://noxytree.github.io/cadence-starter/guides/faceless-video-workflow/)
- [How to detect stale files in media production](https://noxytree.github.io/cadence-starter/guides/stale-files-media-production/)

## Support

If a NoxyTree tool saves you time, you can [support the work on Ko-fi](https://ko-fi.com/noxytree).
